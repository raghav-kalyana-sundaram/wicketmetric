...
------
    ...
    data/output/bowling_careers_full.parquet  – Full career detail (for website; optional bowling_style / bowling_kind when bowling_style_enrichment is enabled in config.yaml)
...
def run_pipeline(
    ...
    incremental : bool, optional
        If True, merge new Cricsheet JSON with ``<output_dir>/.cache/deliveries_pre_xr.parquet``
        when present (see ``--incremental`` CLI flag). Returns ``None`` if there is nothing new.

    Returns
    -------
    dict or None
        Result dict with keys such as ``batting_careers``, ``bowling_careers``, …
        ``None`` only when *incremental* is True and there are no new match files.
    """
    # ── Load configuration ──
    ...
    # ── Step 1: Parse all match files (or merge incremental cache + new JSON) ─
    print("=" * 65)
    print("STEP 1 / 9 — Parsing match files")
    print("=" * 65)
    if incremental:
        from src.incremental_deliveries import (
            merge_new_deliveries_if_possible,
            save_deliveries_cache,
        )

        outcome, df_inc, infos_inc = merge_new_deliveries_if_possible(
            data_dir, output_dir
        )
        if outcome.kind == "no_new":
            print("  Outputs unchanged (incremental sync: no new matches).")
            return None
        if outcome.kind == "merged":
            df, match_infos = df_inc, infos_inc
        else:
            df, match_infos = parse_all_matches(data_dir)
        save_deliveries_cache(output_dir, df)
    else:
        df, match_infos = parse_all_matches(data_dir)
        from src.incremental_deliveries import save_deliveries_cache
    ...
    n_match_display = (
        ...
        else len(match_infos)
    )
    print(f"  Matches    : {n_match_display:>10,}")
    ...
    bse_enabled = config.get("bowling_style_enrichment.enabled", default=False)
    if bse_enabled:
        from pathlib import Path

        from src.bowling_style_enrichment import enrich_bowl_careers_with_espn_styles

        people_csv = str(
            config.get("bowling_style_enrichment.cricsheet_people_csv", default="") or ""
        ).strip()
        cache_dir = str(
            config.get(
                "bowling_style_enrichment.cache_dir",
                default="data/cache/espn_athletes",
            )
        ).strip()
        sleep_s = float(config.get("bowling_style_enrichment.sleep_seconds", default=0.35))
        timeout_s = float(
            config.get("bowling_style_enrichment.timeout_seconds", default=25.0)
        )
        if not people_csv:
            print(
                "  ⚠ bowling_style_enrichment.enabled but cricsheet_people_csv is empty — skipped"
            )
        else:
            pcsv = Path(people_csv)
            if not pcsv.is_absolute():
                pcsv = Path(_PROJECT_ROOT) / pcsv
            cdir = Path(cache_dir)
            if not cdir.is_absolute():
                cdir = Path(_PROJECT_ROOT) / cdir
            if not pcsv.is_file():
                print(
                    f"  ⚠ bowling_style_enrichment: register not found at {pcsv} — skipped"
                )
            else:
                print(
                    "  Enriching bowling styles (Cricsheet register + ESPN athlete API)..."
                )
                bowl_careers = enrich_bowl_careers_with_espn_styles(
                    bowl_careers,
                    people_csv=pcsv,
                    cache_dir=cdir,
                ...
                n_ok = int(bowl_careers["bowling_style_verified"].sum())
                n_id = bowl_careers.loc[
                    bowl_careers["bowling_style_verified"], "bowler_id"
                ].nunique()
                print(
                    f"  ✓ Bowling styles verified for {n_ok:,} rows "
                    f"({n_id:,} distinct bowler_id)"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reuse <output>/.cache/deliveries_pre_xr.parquet: parse only new or replaced "
            "Cricsheet JSON files (by match_id), merge with cached deliveries, then run "
            "the full pipeline (xR through Parquet). Exits without rewriting outputs if "
            "nothing is new. Omit on first run or after deleting the cache."
        ),
    )
    ...
    if results is None:
        print("Done (incremental: no new matches; Parquet left unchanged).")
        sys.exit(0)
