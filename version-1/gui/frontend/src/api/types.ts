...
// ── League team standings (Men/Women × T20I/IPL schema) ───────────
...
export interface LeagueTeamStanding {
  ...
  losses: number;
  win_pct: number | null;
  /** Mean runs per team innings row in the dataset (distinct match/innings). */
  avg_innings_runs: number | null;
}

export interface LeagueTeamStandingsResponse {
...
export interface TeamChipsResponse {
  teams: string[];
}

export interface TeamRecentMatchRow {
  match_id: string;
  date?: string | null;
...
export interface TeamSquadPlayerRow {
  player_id: string;
  player_name: string;
  innings: number;
}

export interface TeamSquadBowlerRow {
  player_id: string;
  player_name: string;
  spells: number;
}
...
export interface TeamDetailResponse {
  team: string;
  display_name: string;
  recent_matches: TeamRecentMatchRow[];
  squad_batters: TeamSquadPlayerRow[];
  squad_bowlers: TeamSquadBowlerRow[];
}

export interface TeamProficientPlayerRow {
  player_id: string;
  player_name: string;
  role: "batter" | "bowler" | "allrounder" | string;
...
export interface TeamProficientPlayersResponse {
  team: string;
  players: TeamProficientPlayerRow[];
}

/** One batting innings: fraction of team total from each scoring type (≈ sums to 1). */
export interface TeamCompositionBattingRow {
  match_id: string;
  innings_num: number;
  date?: string | null;
  label: string;
  ...
  share_twos: number;
  share_ones: number;
  share_running: number;
  share_extras: number;
}

/** One bowling innings: fraction of wickets by dismissal style. */
export interface TeamCompositionBowlingRow {
  match_id: string;
  innings_num: number;
  date?: string | null;
  label: string;
  ...
  share_caught: number;
  share_lbw: number;
  share_run_out: number;
  share_stumped: number;
  share_other: number;
}

export interface TeamCompositionResponse {
  team: string;
  /** When true, ones/twos/threes are from ball-by-ball aggregates; otherwise they stay 0 and running holds the rest. */
  batting_singles_breakdown: boolean;
...
export interface BowlerProfile {
  ...
  recent_team?: string | null;
  /** ESPN-derived bowling style when enrichment has run and identity checks passed */
  bowling_style?: string;
  bowling_kind?: string;
  espn_player_id?: string;
  bowling_style_verified?: boolean;
  /** Optional headshot; CORS required for social export canvas. */
  photo_url?: string | null;
...
/** One outlier beyond Tukey fences for leaderboard distribution. */
export interface LeaderboardDistributionOutlier {
  player_id: string;
  player_name: string;
  value: number;
}

/** Histogram bin for density / violin. */
export interface LeaderboardDistributionBin {
  bin_start: number;
  bin_end: number;
  count: number;
}

/** Request params for leaderboard heatmap (correlation + intensity). */
export interface LeaderboardHeatmapParams extends LeaderboardDistributionFilters {
  max_sample?: number;
  min_pair_obs?: number;
  intensity_top?: number;
  intensity_sort?: string;
  /** Metric keys for the intensity block (max 10). */
  intensity_metrics?: string[];
}

/** Correlation matrix and player×metric intensity from a filtered random sample. */
export interface LeaderboardHeatmapResponse {
  role: string;
  n_players: number;
  correlation_columns: string[];
  correlation: (number | null)[][];
  intensity_player_ids: string[];
  intensity_player_names: string[];
  intensity_metrics: string[];
  intensity_matrix: (number | null)[][];
}

/** Distribution of one metric over the full filtered leaderboard pool. */
export interface LeaderboardDistributionResponse {
  metric: string;
  role: string;
  n: number;
  min: number | null;
  ...
  iqr: number | null;
  outliers: LeaderboardDistributionOutlier[];
  histogram_bins: LeaderboardDistributionBin[];
}
...
/** One calendar year of mean form-tracker scores (0–100). */
...
/** Yearly aggregates for season-to-season slope charts on the player profile. */
export interface PlayerSeasonTrendResponse {
  player_id: string;
  player_name: string;
  role: "bat" | "bowl" | string;
  score_1_label: string;
  score_2_label: string;
  score_3_label: string;
  seasons: PlayerSeasonTrendPoint[];
}
...
/** Filters for GET /api/rankings/{role}/distribution (same pool as leaderboard, no pagination). */
export type LeaderboardDistributionFilters = Pick<
  LeaderboardParams,
  | "country"
  | "archetype"
  | "position_group"
