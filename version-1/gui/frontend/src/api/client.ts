...
export class ApiError extends Error {
  ...
  /** True when the API is up but the DuckDB dataset is missing or not ready (503). */
  get isDataUnavailable(): boolean {
    return this.status === 503;
  }
}

/** True when `error` is an {@link ApiError} for missing / unloaded dataset (HTTP 503). */
export function isDatasetUnavailableError(error: unknown): error is ApiError {
  return error instanceof ApiError && error.isDataUnavailable;
}
...
async function health(
  signal?: AbortSignal,
): Promise<{ status: string; reason?: string; formats?: string[] }> {
...
async function getPlayerSeasonTrend(
  playerId: string,
  role: "bat" | "bowl",
  signal?: AbortSignal,
): Promise<PlayerSeasonTrendResponse> {
  return fetchJson(
    `/api/player/${encodeURIComponent(playerId)}/season-trend`,
    { role },
    { signal },
  );
}
...
/** Distribution (quartiles, histogram, outliers) for one batting metric. */
async function getBattingDistribution(
  metric: string,
  filters: LeaderboardDistributionFilters,
  signal?: AbortSignal,
): Promise<LeaderboardDistributionResponse> {
  return fetchJson(
    "/api/rankings/bat/distribution",
    {
      metric,
      country: filters.country,
      archetype: filters.archetype,
      position_group: filters.position_group,
      modal_slot: filters.modal_slot,
      min_innings: filters.min_innings,
      provisional: filters.provisional,
      activity: filters.activity ?? "active",
      ...(filters.ctx_entry_phase && filters.ctx_entry_phase !== "none"
        ? { ctx_entry_phase: filters.ctx_entry_phase }
        : {}),
      ...(filters.ctx_knockouts_only ? { ctx_knockouts_only: true } : {}),
      ...(filters.ctx_chase_high_rpo ? { ctx_chase_high_rpo: true } : {}),
    },
    { signal },
  );
}

/** Distribution for one bowling metric. */
async function getBowlingDistribution(
  metric: string,
  filters: LeaderboardDistributionFilters,
  signal?: AbortSignal,
): Promise<LeaderboardDistributionResponse> {
  return fetchJson(
    "/api/rankings/bowl/distribution",
    {
      metric,
      country: filters.country,
      archetype: filters.archetype,
      phase_group: filters.phase_group,
      min_innings: filters.min_innings,
      provisional: filters.provisional,
      activity: filters.activity ?? "active",
    },
...
/** Random-sample heatmap: metric correlations + top players × metrics intensity. */
async function getBattingHeatmap(
  p: LeaderboardHeatmapParams,
  signal?: AbortSignal,
): Promise<LeaderboardHeatmapResponse> {
  const im =
    p.intensity_metrics && p.intensity_metrics.length > 0
      ? p.intensity_metrics.join(",")
      : undefined;
  return fetchJson(
    "/api/rankings/bat/heatmap",
    {
      country: p.country,
      archetype: p.archetype,
      position_group: p.position_group,
      modal_slot: p.modal_slot,
      min_innings: p.min_innings,
      provisional: p.provisional,
      activity: p.activity ?? "active",
      max_sample: p.max_sample,
      min_pair_obs: p.min_pair_obs,
      intensity_top: p.intensity_top,
      intensity_sort: p.intensity_sort,
      ...(im ? { intensity_metrics: im } : {}),
      ...(p.ctx_entry_phase && p.ctx_entry_phase !== "none"
        ? { ctx_entry_phase: p.ctx_entry_phase }
        : {}),
      ...(p.ctx_knockouts_only ? { ctx_knockouts_only: true } : {}),
      ...(p.ctx_chase_high_rpo ? { ctx_chase_high_rpo: true } : {}),
    },
    { signal },
  );
}

async function getBowlingHeatmap(
  p: LeaderboardHeatmapParams,
  signal?: AbortSignal,
): Promise<LeaderboardHeatmapResponse> {
  const im =
...
async function getTeamChips(options?: {
  signal?: AbortSignal;
}): Promise<TeamChipsResponse> {
  return fetchJson("/api/teams/chips", {}, { signal: options?.signal });
}

async function getTeamDetail(
  team: string,
  options?: { recent_limit?: number; signal?: AbortSignal },
): Promise<TeamDetailResponse> {
  return fetchJson(
    "/api/teams/detail",
    {
      team,
      recent_limit: options?.recent_limit ?? 20,
    },
    { signal: options?.signal },
  );
}

async function getTeamComposition(
  team: string,
  options?: { limit?: number; signal?: AbortSignal },
): Promise<TeamCompositionResponse> {
  return fetchJson(
    "/api/teams/composition",
    {
      team,
...
async function getTeamProficientPlayers(
  team: string,
  options?: {
    limit?: number;
    min_innings?: number;
...
): Promise<TeamProficientPlayersResponse> {
  return fetchJson(
    "/api/teams/proficient",
    {
      team,
      limit: options?.limit ?? 24,
      min_innings: options?.min_innings ?? 3,
      min_spells: options?.min_spells ?? 3,
    },
    { signal: options?.signal },
  );
}

async function getLeagueTeamStandings(
  options?: {
    min_matches?: number;
    sort?: string;
    order?: string;
...
): Promise<LeagueTeamStandingsResponse> {
  return fetchJson(
    "/api/teams",
    {
      min_matches: options?.min_matches ?? 3,
...
export const api = {
  ...
  getBattingRankings,
  getBowlingRankings,
  getBattingDistribution,
  getBowlingDistribution,
  getBattingHeatmap,
  getBowlingHeatmap,
  getTopPlayers,
  ...
  getPlayersAtVenue,
  getVenueSummary,
  getLeagueTeamStandings,
  getTeamChips,
  getTeamComposition,
  getTeamDetail,
  getTeamProficientPlayers,
