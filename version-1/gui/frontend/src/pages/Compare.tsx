 ...
 *   - 6-axis spider / radar charts with overlaid polygons (one colour per player); mixed-role
 *     compares show batting and bowling spiders side by side
...
type RadarAxisDef = { key: string; label: string; shortLabel: string };

function buildRadarPlayersForCompare(
  profiles: (BatterProfile | BowlerProfile)[],
  axes: readonly RadarAxisDef[],
  allProfiles: (BatterProfile | BowlerProfile)[],
): { name: string; values: number[]; colour: string }[] {
  return profiles.map((profile) => ({
    name: profile.name,
    values: axes.map((axis) => getRadarValue(profile, axis.key)),
    colour: chartColour(allProfiles.findIndex((ap) => ap.id === profile.id)),
  }));
}
      ...
      {players.map((player, pi) => {
        ...
        return (
          <g key={`player-${pi}`}>
            ...
            >
              <title>
                {player.name} profile ({axes.map((a, i) => `${a.label}: ${Math.round(player.values[i] ?? 0)}`).join(", ")})
              </title>
            </polygon>
            {player.values.map((v, i) => {
              ...
              return (
                ...
                >
                  <title>
                    {player.name} — {axis?.label ?? "Metric"}: {Math.round(v)} (0–100)
                  </title>
                </circle>
              );
            })}
      ...
      {axes.map((axis, i) => {
        ...
        return (
          <text
            ...
            fontSize={11}
          >
            <title>{axis.label} — 0–100 scale (higher is stronger on this app metric)</title>
            {axis.shortLabel}
          </text>
        );
      })}
...
export default function Compare() {
  ...
  const isMixedRoles = hasBatters && hasBowlers;

  const batRadarPlayers = useMemo(
    () => buildRadarPlayersForCompare(batters, BATTER_RADAR_AXES, allProfiles),
    [batters, allProfiles],
  );

  const bowlRadarPlayers = useMemo(
    () => buildRadarPlayersForCompare(bowlers, BOWLER_RADAR_AXES, allProfiles),
    [bowlers, allProfiles],
  );

  const compareRadarSubjects = useMemo(
    () => subjectsFromPlayers(allProfiles),
    [allProfiles],
  );
    ...
    <div className="app-page page-stack animate-fade-in">
      ...
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Select 2–4 players to compare side-by-side (spider charts are easiest to read with 2–3).
      ...
      {compareData && canCompare && allProfiles.length >= MIN_PLAYERS && (
        <div className="app-page page-stack">
          ...
          {/* ── Spider / radar — complete multi-stat profile ───────────── */}
          <section className="card p-6">
            <div className="mb-4 flex items-start justify-between gap-2">
              <div className="flex flex-1 min-w-0 flex-col gap-1">
                <h2 className="text-h3 text-text-primary flex flex-wrap items-center gap-2">
                  <BarChart3 size={20} className="text-primary shrink-0" />
                  Spider chart
                  <span className="text-xs font-normal text-text-muted">
                    six 0–100 metrics per role
                  </span>
                </h2>
                <p className="text-xs text-text-secondary max-w-3xl">
                  Overlay polygons compare the same six dimensions for each player—useful for
                  spotting a &ldquo;complete&rdquo; profile (balance vs specialists). Hover axis
                  abbreviations or points for full names and scores.
                </p>
              </div>
              <SocialShareTrigger
                exportRef={compareRadarExportRef}
                filenameBase="compare-spider"
                subjects={compareRadarSubjects}
                subtitle={
                  isMixedRoles
                    ? "Spider charts · batting & bowling"
                    : `Spider chart · ${hasBatters ? "batting" : "bowling"}`
                }
              />
            </div>
                ...
                {isMixedRoles ? (
                  <div className="grid w-full gap-8 lg:grid-cols-2">
                    <div className="flex min-w-0 flex-col items-center gap-2">
                      ...
                      <RadarChart
                        axes={BATTER_RADAR_AXES}
                        players={batRadarPlayers}
                        size={300}
                      />
                    </div>
                    <div className="flex min-w-0 flex-col items-center gap-2">
                      ...
                      <RadarChart
                        axes={BOWLER_RADAR_AXES}
                        players={bowlRadarPlayers}
                        size={300}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center w-full min-w-0">
                    <RadarChart
                      axes={hasBatters ? BATTER_RADAR_AXES : BOWLER_RADAR_AXES}
                      players={hasBatters ? batRadarPlayers : bowlRadarPlayers}
                      size={320}
                    />
                  </div>
                )}
