...
export default function PlayerProfile() {
  ...
  const { data: seasonTrend, isLoading: seasonTrendLoading } = usePlayerSeasonTrend(
    id,
    formRole,
    { enabled: !!id && activeRole !== null },
  );
    ...
    <div className="app-page page-stack pb-8">
      ...
      {isBatView ? (
        <BatterProfileView
          ...
          seasonTrendLoading={seasonTrendLoading}
...
interface BatterProfileViewProps {
  ...
  formData: any;
  formLoading: boolean;
  seasonTrend: PlayerSeasonTrendResponse | undefined;
  seasonTrendLoading: boolean;
  inningsData: any;
  logPage: number;
  logPerPage: number;
...
/** Before/after slope: two calendar years × four 0–100 form metrics (Recharts categorical lines). */
function SeasonSlopeSection({
  trend,
  loading,
  role,
}: {
  trend: PlayerSeasonTrendResponse | undefined;
  loading: boolean;
  role: "bat" | "bowl";
}) {
  const seasons = trend?.seasons ?? [];
  const [leftY, setLeftY] = useState<number | null>(null);
  const [rightY, setRightY] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    setLeftY(ys[ys.length - 2]!);
    setRightY(ys[ys.length - 1]!);
  }, [seasons]);

  const chartData = useMemo(() => {
    if (leftY == null || rightY == null || leftY === rightY) return [];
    const a = seasons.find((s) => s.year === leftY);
    const b = seasons.find((s) => s.year === rightY);
    if (!a || !b) return [];
    return [
      {
        period: String(leftY),
        composite: a.avg_composite ?? null,
        score_1: a.avg_score_1 ?? null,
        score_2: a.avg_score_2 ?? null,
        score_3: a.avg_score_3 ?? null,
      },
      {
        period: String(rightY),
        composite: b.avg_composite ?? null,
        score_1: b.avg_score_1 ?? null,
        score_2: b.avg_score_2 ?? null,
        score_3: b.avg_score_3 ?? null,
      },
    ];
  }, [seasons, leftY, rightY]);

  const l1 = _cap(
    trend?.score_1_label ?? (role === "bat" ? "acceleration" : "accuracy"),
  );
  const l2 = _cap(
    trend?.score_2_label ?? (role === "bat" ? "power" : "control"),
  );
  const l3 = _cap(
    trend?.score_3_label ?? (role === "bat" ? "control" : "threat"),
  );

  const rowL = seasons.find((s) => s.year === leftY);
  const rowR = seasons.find((s) => s.year === rightY);

  if (loading && !seasons.length) {
    return (
      <section className="card p-6">
        <SectionTitle icon={<ArrowRightLeft size={18} />} title="Season slope" />
        <p className="text-sm text-text-muted mt-4">Loading season breakdown…</p>
      </section>
    );
  }
  ...
  if (seasons.length < 2) {
    return (
      <section className="card p-6">
        <SectionTitle icon={<ArrowRightLeft size={18} />} title="Season slope" />
        <p className="text-sm text-text-muted mt-4 max-w-xl">
          Need rolling-form data in at least two calendar years (two points per line for a
          before/after slope). This appears once the pipeline has built{" "}
          <span className="text-text-secondary">bat_form</span> /{" "}
          <span className="text-text-secondary">bowl_form</span> for this player in this format.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="card p-6">
      <SectionTitle icon={<ArrowRightLeft size={18} />} title="Season slope" />
      <p className="text-xs text-text-muted mt-1 mb-4 max-w-3xl">
        Each year is the mean of the same 0–100 rolling form scores as the tracker (grouped by
        calendar year). Choose an earlier and later season to read improvement or decline across
        composite and sub-skills.
      </p>
      <div className="flex flex-wrap items-end gap-4 mb-4">
        <label className="flex flex-col gap-1 text-xs text-text-muted">
          Earlier
          <select
            className="rounded-lg border border-surface-elevated bg-surface px-2 py-1.5 text-sm text-text-primary"
            value={leftY ?? ""}
            onChange={(e) => setLeftY(Number(e.target.value))}
          >
            {seasons.map((s) => (
              <option key={s.year} value={s.year}>
                {s.year} ({s.sample_points} pts)
              </option>
            ))}
          </select>
        </label>
        <span className="text-text-muted pb-2 text-xs" aria-hidden>
          →
        </span>
        <label className="flex flex-col gap-1 text-xs text-text-muted">
          Later
      ...
      {chartData.length === 2 ? (
        <>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 12, right: 16, left: 0, bottom: 8 }}>
                ...
                <Line
                  ...
                  strokeWidth={2.5}
                  dot={{ r: 4, fill: CHART_COLOURS[0], strokeWidth: 0 }}
                  connectNulls
                />
                <Line
                  type="linear"
                  ...
                  strokeWidth={2}
                  dot={{ r: 3, fill: SUB_COLOURS.score_1, strokeWidth: 0 }}
                  connectNulls
                />
                <Line
                  type="linear"
                  ...
                  strokeWidth={2}
                  dot={{ r: 3, fill: SUB_COLOURS.score_2, strokeWidth: 0 }}
                  connectNulls
                />
                <Line
                  type="linear"
                  ...
                  strokeWidth={2}
                  dot={{ r: 3, fill: SUB_COLOURS.score_3, strokeWidth: 0 }}
                  connectNulls
                />
              </LineChart>
            </ResponsiveContainer>
          ...
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="rounded-lg border border-surface-elevated/60 px-2 py-2">
              <div className="text-text-muted mb-0.5">Composite Δ</div>
              <div className="font-score tabular-nums text-text-primary">
                {fmtSlopeDelta(rowL?.avg_composite, rowR?.avg_composite)}
              </div>
            </div>
            <div className="rounded-lg border border-surface-elevated/60 px-2 py-2">
              <div className="text-text-muted mb-0.5">{l1} Δ</div>
              <div className="font-score tabular-nums text-text-primary">
                {fmtSlopeDelta(rowL?.avg_score_1, rowR?.avg_score_1)}
              </div>
            </div>
            <div className="rounded-lg border border-surface-elevated/60 px-2 py-2">
              <div className="text-text-muted mb-0.5">{l2} Δ</div>
              <div className="font-score tabular-nums text-text-primary">
                {fmtSlopeDelta(rowL?.avg_score_2, rowR?.avg_score_2)}
              </div>
            </div>
            <div className="rounded-lg border border-surface-elevated/60 px-2 py-2">
              <div className="text-text-muted mb-0.5">{l3} Δ</div>
              <div className="font-score tabular-nums text-text-primary">
                {fmtSlopeDelta(rowL?.avg_score_3, rowR?.avg_score_3)}
              </div>
            </div>
          </div>
        </>
      ) : (
        <p className="text-sm text-text-muted">Pick two different years above to compare.</p>
      )}
    </section>
  );
}
