            ...
            for ev in events:
                ...
                if isinstance(comps, list):
                    for row in comps:
                        ...
                        disp = str(
                            row.get("displayName") or row.get("name") or ""
                        ).strip()
                        nm = abbr or disp
...
def extract_match_detail_for_ui(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for t in teams_out:
        ...
        if ab:
            label_map[ab] = short
    for sec in matchcard_sections:
        tn = str(sec.get("team_name") or "").strip()
        if tn and tn in label_map:
            sec["team_name"] = label_map[tn]
