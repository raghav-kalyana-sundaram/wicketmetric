...
class BowlerProfile(BaseModel):
    ...
    bowling_style: str = ""
    bowling_kind: str = "unknown"  # pace | spin | unknown
    espn_player_id: str = ""
    bowling_style_verified: bool = False
...
class LeaderboardDistributionOutlier(BaseModel):
    """Single outlier point outside Tukey fences (beyond 1.5×IQR)."""
...
class LeaderboardDistributionBin(BaseModel):
    """One histogram bin for density / violin rendering."""
...
class LeaderboardHeatmapResponse(BaseModel):
    """Correlation matrix + optional player×metric intensity (normalized) for a filtered pool."""

    role: str = "bat"
    n_players: int = 0
    correlation_columns: list[str] = []
    correlation: list[list[float | None]] = []
    intensity_player_ids: list[str] = []
    intensity_player_names: list[str] = []
    intensity_metrics: list[str] = []
    intensity_matrix: list[list[float | None]] = []


class LeaderboardDistributionResponse(BaseModel):
    """Distribution summary for one leaderboard metric (all filtered players)."""
    ...
    histogram_bins: list[LeaderboardDistributionBin] = []
...
class PlayerSeasonTrendPoint(BaseModel):
    """One calendar year of aggregated rolling-form scores (0–100 scale)."""

    year: int
    sample_points: int = 0
    avg_composite: float | None = None
    avg_score_1: float | None = None
    avg_score_2: float | None = None
    avg_score_3: float | None = None


class PlayerSeasonTrendResponse(BaseModel):
    """Yearly means of form-tracker scores for season-to-season slope charts."""
    ...
    score_2_label: str = "power"
    score_3_label: str = "control"
    seasons: list[PlayerSeasonTrendPoint] = Field(default_factory=list)
...
# ── League team standings (per format schema) ────────────────────


class LeagueTeamStanding(BaseModel):
    """One team's aggregate record in the active dataset slice."""
    ...
    avg_innings_runs: float | None = None


class LeagueTeamStandingsResponse(BaseModel):
    """Paginated team list with W/L and mean innings total when batting first in the row set."""

    teams: list[LeagueTeamStanding] = Field(default_factory=list)
    total: int = 0
    page: int = 1
    per_page: int = 50
    total_pages: int = 0
...
class TeamChipsResponse(BaseModel):
    """All side names in the format slice (for horizontal team navigation)."""

    teams: list[str] = Field(default_factory=list)


class TeamRecentMatchRow(BaseModel):
    match_id: str = ""
    date: str | None = None
    opposition: str | None = None
    """W | L | T | NR from this team's perspective."""
    result: str = "NR"
    venue: str | None = None


class TeamSquadPlayerRow(BaseModel):
    player_id: str = ""
    player_name: str = ""
    innings: int = 0


class TeamSquadBowlerRow(BaseModel):
    player_id: str = ""
    player_name: str = ""
    spells: int = 0
...
class TeamDetailResponse(BaseModel):
    """Hub payload for one team: recent results and squad lists."""

    team: str = ""
    display_name: str = ""
    recent_matches: list[TeamRecentMatchRow] = Field(default_factory=list)
    squad_batters: list[TeamSquadPlayerRow] = Field(default_factory=list)
    squad_bowlers: list[TeamSquadBowlerRow] = Field(default_factory=list)


class TeamProficientPlayerRow(BaseModel):
    """One player ranked for this franchise / national side."""

    player_id: str = ""
    player_name: str = ""
    role: str = "batter"  # batter | bowler | allrounder
...
class TeamProficientPlayersResponse(BaseModel):
    """Most proficient players for a side (career WAR × team-scoped volume)."""

    team: str = ""
    players: list[TeamProficientPlayerRow] = Field(default_factory=list)


class TeamCompositionBattingRow(BaseModel):
    """One team batting innings: share of team total runs by scoring type (sums to ~1)."""
    ...
    share_sixes: float = 0.0
    share_fours: float = 0.0
    share_threes: float = 0.0
    share_twos: float = 0.0
    share_ones: float = 0.0
    share_running: float = 0.0
    share_extras: float = 0.0


class TeamCompositionBowlingRow(BaseModel):
    """One bowling innings for this side: share of wickets by dismissal style."""
    ...
    label: str = ""
    wickets: int = 0
    share_bowled: float = 0.0
    share_caught: float = 0.0
    share_lbw: float = 0.0
    share_run_out: float = 0.0
    share_stumped: float = 0.0
    share_other: float = 0.0


class TeamCompositionResponse(BaseModel):
    """Stacked-area data: batting run mix and bowling wicket mix over recent innings."""

    team: str = ""
    batting_singles_breakdown: bool = False
    batting: list[TeamCompositionBattingRow] = Field(default_factory=list)
    bowling: list[TeamCompositionBowlingRow] = Field(default_factory=list)
