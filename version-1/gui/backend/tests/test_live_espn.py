...
def test_build_header_prefers_team_abbreviation_for_score_line() -> None:
    raw = {
        "sports": [
            {
                "leagues": [
    ...
    s = build_events_summary_from_header(raw)
    assert len(s) == 1
    assert s[0]["score_line"] == "RR 120/4 · MI 118/8"
    assert s[0]["competitors"][0]["name"] == "RR"
    assert s[0]["competitors"][1]["name"] == "MI"
