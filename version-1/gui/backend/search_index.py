...
def build_search_index(store: "DataStore") -> TrigramIndex:
    """Build the trigram search index from the DataStore (legacy pandas path).
...
def build_search_index_from_db(conn, fmt: str) -> TrigramIndex:
    """Build the trigram search index from DuckDB.

    Queries batting and bowling careers from the given format schema,
    feeds rows into the same ``_extract_*`` functions used by the
    pandas path.
    ...
    fmt : str
        Format schema name (e.g. ``mens_t20i``).
    ...
    TrigramIndex
        Ready-to-query search index.
    """
    from db import safe_fmt

    f = safe_fmt(fmt)
    index = TrigramIndex()

    bat_df = conn.execute(f"SELECT * FROM {f}.bat_careers").fetchdf()
    bat_entries = _extract_batting_entries(bat_df)
    for entry in bat_entries:
        index.add(entry)

    bowl_df = conn.execute(f"SELECT * FROM {f}.bowl_careers").fetchdf()
    bowl_entries = _extract_bowling_entries(bowl_df)
    for entry in bowl_entries:
        index.add(entry)
