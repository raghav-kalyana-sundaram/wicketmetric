...
function MatchcardBlock({ sec }: { sec: EspnMatchcardSection }): JSX.Element {
  const subtitle = [sec.team_name, sec.innings_number != null ? `Inn. ${sec.innings_number}` : ""]
    .filter(Boolean)
    .join(" · ");
  return (
...
function liveScoresFromTeams(teams: EspnCricketMatchDetail["teams"]): string {
  return teams
    .map((t) => {
      const nm = (t.name || t.abbreviation || "").trim();
          ...
          {(data.detail.matchcard_sections?.length ?? 0) > 0 ? (
            <section className="section-card section-card-body" aria-label="Live scorecard">
              ...
              {data.detail.matchcard_sections!.map((sec, idx) => (
                <MatchcardBlock key={`${sec.headline}-${sec.innings_number ?? ""}-${idx}`} sec={sec} />
              ))}
            </section>
          ) : null}
                  ...
                  <tbody className="text-text-secondary">
                    {data.detail.fall_of_wickets.map((w, i) => (
                      <tr key={`fow-${i}`} className="border-b border-surface-elevated/60">
                        <td className="py-2 pr-3 align-top">{w.team_name || "—"}</td>
